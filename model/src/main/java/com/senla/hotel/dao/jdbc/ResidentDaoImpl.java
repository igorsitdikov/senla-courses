...
public class ResidentDaoImpl extends AbstractDao<Resident, Long> implements ResidentDao {
    ...
    @Override
    protected void prepareStatementForInsert(final PreparedStatement statement, final Resident object)
    ...
    @Override
    protected void prepareStatementForUpdate(final PreparedStatement statement, final Resident object)
        throws PersistException {
        try {
            statement.setString(1, object.getFirstName());
            statement.setString(2, object.getLastName());
    ...
    public List<Resident> getAllSortedBy(final SortField sortField) throws PersistException {
        final List<Resident> list;
        String sql = getSelectQuery();
        switch (sortField) {
            case NAME:
                sql += " ORDER BY rt_first_name, rt_last_name ASC";
                break;
            case CHECK_OUT_DATE:
                sql += " ORDER BY h_check_out ASC";
                break;
            default:
