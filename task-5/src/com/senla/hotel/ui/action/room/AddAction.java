...
public class AddAction implements IAction {
    ...
    public void execute() {
        ...
        try {
            ...
            Stars stars = integerToStar(InputDataReader.getIntegerInput(scanner, "Input the Room rate from 1 to 9, where" +
                    "1 - JUNIOR SUIT, " +
                    "2 - SUIT, " +
                    "3 - DE LUX, " +
                    "4 - DUPLEX, " +
            ...
            Accommodation accommodation = integerToAccommodation(InputDataReader.getIntegerInput(scanner, "Input the Room accommodation, where" +
                    "1 - SINGLE RESIDENT, " +
                    ...
                    "10 - TRIPLE RESIDENTS WITH 2 CHILDREN...", Accommodation.values().length));
            BigDecimal dailyPrice = BigDecimal.valueOf(InputDataReader.getDoubleInput(scanner, "Input the Room daily price..."));
            RoomStatus status = integerToStatus(InputDataReader.getIntegerInput(scanner,
                    "Input the Room status, where\n " +
                            "\t1 = VACANT, " +
                            "\t2 = OCCUPIED, " +
                            "\t3 = REPAIR...", RoomStatus.values().length));
            HotelController.getInstance().addRoom(new Room(roomNumber, stars, accommodation, dailyPrice, status));
